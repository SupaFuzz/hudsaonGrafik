/*
    tcApplicationCore.js        12/29/20   Amy Hicox <amy@hicox.com>
*/
...
/*
    CLASS   tcLogMessage
*/
class tcLogMessage extends tcChildClass {

// minimal constructor
constructor(args, defaults, callback){
    let _classDefaults = tcObjectCore.mergeClassDefaults({
        _version:           1,
        _className:         'tcLogMessage',
...
/*
    ...
    tcLog objects have these functions:
        ...
        * log(message, fatal, flushImmediate)
          this creates a new log entry.
                ...
                * fatal             <bool> default: false
                  if fatal is set true, we will write the log, call flush() and throw a tcException
        ...
        * flush()
          ...
          of the messageQueue
*/
class tcLog extends tcChildClass {
...
log(message, fatal, flushImmediate) {
    ...
    this.messageQueue.push(new tcLogMessage({
    ...
    if (_fatal){
        throw(new tcException({
            message:        `tcLog received a fatal log message: ${_message}`,
            messageNumber:   17,
            thrownBy:       'tcLog/log'
        }));
    }
...
/*
    ...
    attributes:
        * localStorageKey       <string> default: tcApplicationCore
        ...
        * logLimit              <int>    default: 0
          pass this through to the tcLog constructor (max number of lines to keep in a logInstance)
        ...
        * name                  <string> default: tcApplicationCore
          the name of the application

        * version               <int>    default: whatever version of tc this is
        ...
        * threads               <obj>   default: {}
          ...
          it's presumed that the threads will use the tcWorkerThread infrastructure

    functions:
        * log(message, fatal, flushImmediate)
          passthrough to tcLog instance
...
*/
class tcApplicationCore extends tcNetworkUtility {

// minimal constructor
constructor(args, defaults, callback){
    let _classDefaults = tcObjectCore.mergeClassDefaults({
        _version:             2,
        _className:           'tcApplicationCore',
        _isCrashed:           false,
        _logLimit:            0,
        _logInstanceLimit:    0,
        _threadHandles:       {},
    ...
    // set up tcLog instance
    let that = this;
    this.startTime = this.epochTimestamp(true);
    this.logHistory[this.startTime] = new tcLog({
    ...
    Object.keys(that.threads).forEach(function(threadName){
        ...
        try {
            that._threadHandles[threadName] = new Worker(that.threads[threadName]);
    ...
    if ((that.threads.hasOwnProperty('serviceWorker')) && (that.enableServiceworker === true) && ('serviceWorker' in navigator)){
        ...
        navigator.serviceWorker.onerror   = function(evt){ that.handleThreadError({threadName: threadName, event: evt}); };
    }
}
...
handleThreadMessage(args){
    if (
        ...
        switch(args.event.data.type){
            ...
            case 'log':
                this.log(
                    `thread [${args.threadName}]: ${args.event.data.data.message}`,
    ...
    if (! this.cantSave){
        try {
            ...
            throw(new tcException({
                ...
                messageNumber: 0,
                thrownBy:      'tcApplicationCore/save',
                fatal:         true
            }));
        }
    }
    ...
    if (localStorage.hasOwnProperty(this.localStorageKey)){
        ...
        try {
            ...
            Object.keys(previousSelf.logHistory).forEach(function(runInstanceTime){
                ...
                self.logHistory[runInstanceTime] = new tcLog({
            ...
            */
            ...
            throw(new tcException({
                ...
                messageNumber: 18,
                thrownBy:      'tcApplicationCore/restore',
                fatal:         false
            }));
            return(false);
        }
